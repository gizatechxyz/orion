...
fn u32_tensor_3x3x3_helper() -> Tensor<u32> {
    let mut sizes: Array<u32> = array![3, 2, 3];
...
fn test_array() -> Tensor<u32> {
    let mut sizes: Array<u32> = array![0, 3, 4];
    let mut data: Array<u32> = array![];
...
fn test_reshape() {
    // let mut data = u32_tensor_3x3x3_helper();
    let mut data = test_array();

    // let mut sizes: Array<u32> = array![3, 2, 3];

    let y = data.reshape(array![0, 4, 3].span(), Option::None);
