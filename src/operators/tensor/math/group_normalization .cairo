...
fn group_normalization<T,
    ...
    let group_size = ((*self.shape.at(1)).into()) / num_groups;
    ...
    x_normalized = x_normalized.reshape(*(self.shape).into(), false);
    ...
    let mut scale = scale.reshape(target_shape: new_scale_shape.span(), false);
    let mut bias = bias.reshape(target_shape: new_scale_shape.span(), false); // since bias and scale have same shape
    ...
    if new_scale_shape.len() != x_normalized.shape.len() {
        ...
        scale = scale.reshape(target_shape: expanded_new_scale_shape.span(), false);
        bias = bias.reshape(target_shape: expanded_new_bias_shape.span(), false);
    }
