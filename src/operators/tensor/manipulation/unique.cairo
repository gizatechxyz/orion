...
>(
    ...
    match axis {
        Option::Some(axis) => {
            ...
            let unique_tensors: Array<Tensor<T>> = get_unique_subtensors(self, axis, rank);
        },
        Option::None => {
            loop {
...
/// Returns in an array all the unique subtensors of the passed tensor.
fn get_unique_subtensors<
    T,
    impl TCopy: Copy<T>,
    impl TDrop: Drop<T>,
    impl TTensor: TensorTrait<T>,
...
>(
    ...
    loop {
        ...
        if !unique_tensors.span().contains(sub_tensor) {
            unique_tensors.append(sub_tensor);
        }
        idx += 1;
    };
    unique_tensors
