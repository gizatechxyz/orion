...
fn tensor_pad<
    ...
    +Drop<T>,
    +TensorTrait<T>,
>(input_data: Tensor<T>, mut pad_slices: Array<Array<usize>>, shape: Array<usize>, zero: T) -> Tensor<T> {
    ...
    loop {
        ...
        if i == 0 {
            if shape_i != input_data_shape_i {
                ...
                res = make_zero_array(*slice.at(0) * count, zero).concat(
                    temp
                ).concat(
        ...
        if shape_i != input_data_shape_i {
            let mut arr_list: Array<Array<T>> = make_array_from_dim(res, count * *input_data_shape_i);
            res = ArrayTrait::<T>::new().span();
            loop {
                match arr_list.pop_front() {
                    Option::Some(mut arr) => {
                        ...
                        res = res.concat(make_zero_array((*shape_i - *slice.at(slice_len - 1) - 1) * count, zero));
                    },
                    Option::None(_) => { break (); }
                };
            };
...
fn tensor_crop<
    ...
    +Drop<T>,
    +TensorTrait<T>,
>(input_data: @Tensor<T>, mut crop_slices: Array<Array<usize>>) -> Tensor<T> {
