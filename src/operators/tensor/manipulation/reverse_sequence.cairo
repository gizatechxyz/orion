...
fn reverse_index(
    shape: Span<usize>, sequence_lens: Tensor<usize>, control: bool 
) -> Array<usize> {
    ...
    if control {
        //[i, slice]
        assert!(sequence_lens.data.len() <= x,"The length of sequence_lens cannot exceed batch_axis");
        let mut i: usize = 0;
        loop {
            if i >= x {
                break;
            ...
            assert!(reverse <= y && reverse >= 1, "sequence_lens must be greater than one and less than batch_size");
        ...
        // [slice, i]
        assert!(sequence_lens.data.len() <= y,"The length of sequence_lens cannot exceed time_axis");
        ...
        loop {
            ...
            let reverse: usize = *(sequence_lens.data).at(i);
            assert!(reverse <= x && reverse >= 1, "sequence_lens must be greater than one and less than batch_size");
