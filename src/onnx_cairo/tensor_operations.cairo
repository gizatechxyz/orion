...
// ################## FIX OPERATIONS #################################
func arrays_mul_fix{range_check_ptr}(array_1: Fix64x61*, array_2: Fix64x61*, size: felt) -> (
    res: Fix64x61*
) {
    alloc_locals;
...
func arrays_mul_fix__inner{range_check_ptr}(
    array_1: Fix64x61*, array_2: Fix64x61*, res: Fix64x61*, size: felt
) {
    alloc_locals;
    if (size == 0) {
        return ();
...
func arrays_add_fix{range_check_ptr}(array_1: Fix64x61*, array_2: Fix64x61*, size: felt) -> (
    res: Fix64x61*
) {
    alloc_locals;
    let (local res: Fix64x61*) = alloc();
...
func arrays_add_fix__inner{range_check_ptr}(
    array_1: Fix64x61*, array_2: Fix64x61*, res: Fix64x61*, size: felt
) {
    alloc_locals;
    if (size == 0) {
