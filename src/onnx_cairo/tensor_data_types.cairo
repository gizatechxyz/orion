...
from src.small_math import Fix64x61, Double, Double_to_Fix
...
// ############### FIX TENSOR INIT #######################
func init_tensor_from_double{range_check_ptr}(
    dim_size: felt, dims: felt*, elements_size: felt, elements_double: Double*
) -> (res: TensorFix) {
    alloc_locals;
    let vars: felt* = alloc();
...
func array_from_double_to_fix{range_check_ptr}(
    array_double: Double*, array_fix: Fix64x61*, size: felt, index: felt
) {
    if (index == size) {
        return ();
    }
...
// ############### FELT TENSOR INIT #######################
func init_tensor_fix(dim_size: felt, dims: felt*, elements_size: felt, elements: Fix64x61*) -> (
    res: TensorFix
) {
    alloc_locals;
    let vars: felt* = alloc();
...
// ############### FELT TENSOR INIT #######################
func init_tensor(dim_size: felt, dims: felt*, elements_size: felt, elements: felt*) -> (
    res: Tensor
) {
    alloc_locals;
    let vars: felt* = alloc();
