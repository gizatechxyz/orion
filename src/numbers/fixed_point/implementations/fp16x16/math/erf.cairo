...
use orion::numbers::fixed_point::implementations::fp16x16::core::{
    ONE, FP16x16, FixedTrait, FP16x16Impl, FP16x16PartialOrd
};
...
fn get_key(mag: u32, erf_computational_accuracy: u32, round_check_number: u32) -> u32{
    ...
    if (round_number % round_check_number) >= 5{
	...
	if (origin_number % erf_computational_accuracy) >= 5{
...
fn erf(x: FP16x16) -> FP16x16{
    ...
    if x.mag < MAX_ERF_NUMBER {
        ...
        erf_value = erf_table.get(round_number);
    } else {
        erf_value = ONE;
    }
