...
func elem_x_elem_add (tensor_1: Tensor, tensor_2: Tensor) -> (res: Tensor):
    ...
    let (res_elements: felt*) = arrays_add_felt(tensor_1.elements, tensor_2.elements, tensor_1.elements_size) 
    let (res : Tensor) = init_tensor(tensor_1.dims_size, tensor_1.dims, tensor_1.elements_size, res_elements)
    return (res = res)
end
...
func elem_x_elem_mul (tensor_1: Tensor, tensor_2: Tensor) -> (res: Tensor):
    ...
    let (res_elements: felt*) = arrays_mul_felt(tensor_1.elements, tensor_2.elements, tensor_1.elements_size) 
    let (res : Tensor) = init_tensor(tensor_1.dims_size, tensor_1.dims, tensor_1.elements_size, res_elements)
    return (res = res)
end
...
func check_elem_x_elem_fix_req (tensor_1: TensorFix, tensor_2: TensorFix) -> (res: felt):
    ...
    tempvar check_condition_sizes = (tensor_1.dims_size - tensor_2.dims_size + 1) + (tensor_1.elements_size - tensor_2.elements_size + 1)
...
func check_equal_tensors (tensor_1: Tensor, tensor_2: Tensor) -> (res: felt):
    alloc_locals
    let (local check_condition_dims) = equal_arrays(tensor_1.dims, tensor_2.dims, tensor_1.dims_size)
    let (local check_condition_elements) = equal_arrays(tensor_1.elements, tensor_2.elements, tensor_1.elements_size)
    tempvar check_condition_sizes = (tensor_1.dims_size - tensor_2.dims_size + 1) + (tensor_1.elements_size - tensor_2.elements_size + 1)
...
func axis_sum_fix (tensor_fix: TensorFix, dims_permutation: felt*) -> (res: TensorFix):
    ...
    let tensor_felt: Tensor = Tensor (tensor_fix.dims_size, tensor_fix.dims, tensor_fix.vars, tensor_fix.elements_size, elements_felt)
    let (axis_sum_res) = axis_sum(tensor_felt, dims_permutation)
    let elements_res: Fix64x61* = cast(axis_sum_res.elements, Fix64x61*)
    let tensor_res_fix: TensorFix = TensorFix(axis_sum_res.dims_size, axis_sum_res.dims, axis_sum_res.vars, axis_sum_res.elements_size, elements_res)
...
func axis_sum (tensor: Tensor, dims_permutation: felt*) -> (res: Tensor):
    ...
    let (tensor_elements_permuted) = permutate_array(tensor.elements, looped_indexes_elements, tensor.elements_size)
    ...
    axis_sum__inner (tensor_elements_permuted, res_elements, looped_indexes_elements, tensor.elements_size, sum_vector_size, 0)
    ...
    let (res: Tensor) = init_tensor(tensor.dims_size - 1, new_dims, new_elements_size, res_elements) 
    return (res = res)
end

func axis_sum__inner (tensor_elements_permuted: felt*, res: felt*, looped_indexes: felt*, looped_indexes_size: felt, sum_size: felt, index: felt):
    ...
    axis_sum__inner (tensor_elements_permuted, res, looped_indexes, looped_indexes_size, sum_size, index + sum_size)
    return()
end

func sum_n_elements (tensor_elements_permuted: felt*, looped_indexes: felt*, initial_index: felt, n: felt) -> (res: felt):
    alloc_locals
    if n == 0:
        return(res = 0)
    end
...
func loop (tensor: Tensor, dims_permutation: felt*) -> (res: Tensor):
    ...
    let (res_elements: felt*) = loop__inner (tensor, res_elements, 1, dims_permutation, tensor.dims_size, 0)
...
func loop__inner (
        tensor: Tensor, current_expansion: felt*, current_expansion_size: felt, dims_permutation: felt*, 
        permutation_size: felt, permutation_index: felt) -> (res_elements: felt*):
    alloc_locals
    ...
    expand_vector (expanded_elements, tensor, current_expansion, current_expansion_size, current_dim, current_var, 0, 0)
    tempvar expanded_elements_size = current_expansion_size * current_dim
    let (res_elements) = loop__inner (tensor, expanded_elements, expanded_elements_size, dims_permutation, permutation_size, permutation_index + 1)
...
func expand_vector (
    ...
    expand_vector (res, tensor, initial_elements, initial_elements_size, vector_size, step, res_size + vector_size, index_initial_elements + 1)
    return()
end

# TODO: just pass dimensions, not the whole tensor
# GETS VECTORS OF INDEXES
func get_index_vector_from_tensor (tensor: Tensor, initial_element: felt, vector_size: felt, step: felt) -> (res: felt*):
    alloc_locals
    let res: felt* = alloc()
    get_index_vector_from_tensor__inner (tensor.elements, initial_element, res, vector_size, step, 0)
    return(res = res)
end

func get_index_vector_from_tensor__inner (tensor_elements: felt*, initial_element: felt, res: felt*, res_size: felt, step: felt, index):
    if index == res_size:
        return()
    end
    tempvar index_in_tensor = initial_element + index * step
    assert [res + index] = index_in_tensor
    get_index_vector_from_tensor__inner (tensor_elements, initial_element, res, res_size, step, index + 1)
    return()
end

# GETS VECTORS OF ACTUAL VALUES IN THE TENSOR
func get_vector_from_tensor (tensor: Tensor, initial_element: felt, vector_size: felt, step: felt) -> (res: felt*):
    alloc_locals
    let res: felt* = alloc()
...
func get_vector_from_tensor__inner (tensor_elements: felt*, initial_element: felt, res: felt*, res_size: felt, step: felt, index):
    if index == res_size:
        return()
    end
    tempvar index_in_tensor = initial_element + index * step
...
func remove_element__inner (array: felt*, size: felt, index_ini: felt, res: felt*, index_fin: felt, index_removed: felt):
    if index_ini == size:
        return()
    end
